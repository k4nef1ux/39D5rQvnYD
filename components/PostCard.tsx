...
//  - a PRODUCT card (type "gift" or anything with a price/buyUrl): a large
//    vitrine image on a lit pedestal, then "the plaque" - gold hairline, serif
//    title left, champagne price right, meta + shop below. One card per row
//    in the stream: the image IS the post.
...
function ProductCard({ post }: { post: PostCardData }) {
  ...
  return (
    <div className="card-product">
      ...
      <div className="pcard-plaque">
        <div className="pcard-head">
          <Link href={href} className="pcard-title">{post.title}</Link>
          {post.price ? <span className="pcard-price">{post.price}</span> : null}
        </div>
        {post.description ? <p className="pcard-desc">{post.description}</p> : null}
        <div className="pcard-foot">
          <span className="pcard-meta">
            {typeof post.rating === "number" ? (
              <span aria-label={`rated ${post.rating} out of 5`}>{post.rating.toFixed(1)} / 5</span>
            ) : null}
            {typeof post.rating === "number" && post.merchant ? " · " : null}
            {post.merchant || null}
          </span>
          {post.buyUrl ? (
